...
class Purchase_Model_DbTable_Dbpurchasepprove extends Zend_Db_Table_Abstract
{
	...
	function getAllPurchaseOrder($search){
		...
		is_recieved
		FROM `tb_purchase_order` WHERE is_approved IN(1,2)";
	...
	public function addPurchaseApproved($data)	{
		$db = $this->getAdapter();
		$db->beginTransaction();
		try{
			...
			$pending=3;
			$is_purchase_order=1;
			if($data['approved_name']==2){$pending=2;$is_purchase_order=0;}
			$arr=array(		
