...
class Purchase_Model_DbTable_DbPriceCompare extends Zend_Db_Table_Abstract
{
	...
	function getProductRequestDetail($id){
		...
		$sql = "SELECT 
				  ...
				  pr.`number_request`,
				  pr.branch_id,
				  (SELECT pl.`name` FROM `tb_plan` AS pl WHERE pl.id=pr.`plan_id` LIMIT 1) AS plan,
				  (SELECT sl.name FROM `tb_sublocation` AS sl WHERE sl.id=pr.`branch_id` LIMIT 1) AS branch,
				  (SELECT i.item_name FROM `tb_product` AS i WHERE i.id = p.`pro_id` LIMIT 1) AS item_name,
				  (SELECT bd.`materail_price` FROM `tb_boqdetail` AS bd WHERE bd.`pro_id`=p.`pro_id` AND bd.`boq_id`=(SELECT b.`id` FROM `tb_boq` AS b WHERE b.`project_name`=pr.`plan_id` LIMIT 1) LIMIT 1) AS boq_price,
				  p.`qty`,
				  (SELECT pl.`price` FROM `tb_prolocation` AS pl WHERE pl.`location_id`=pr.`branch_id` AND pl.pro_id=p.`pro_id` LIMIT 1) AS cost_price,
